...
const toastBootstrap = bootstrap.Toast.getOrCreateInstance(cartToast); // Inicializar el objeto Toast
...
function agregarProducto(nombre, precio, imagen, boton) {
  // 1. Añadir al array de estado
  productosEnCesta.push({ nombre, precio, imagen });
  
  // 2. Actualizar totales
  suma += precio;
  
  // 3. Guardar estado y actualizar vista
  localStorage.setItem('cesta', JSON.stringify(productosEnCesta));
  localStorage.setItem('totalCesta', suma.toFixed(2));
  actualizarCesta();
  
  // 4. Mostrar feedback visual (popup)
  mostrarPopup(boton);
}
...
botonesComprar.forEach(boton => {
  boton.addEventListener('click', e => {
    ...
    const imagen = producto.querySelector('.coleccion-img').src; // <--- SELECCIONA LA IMAGEN
    const precio = parseFloat(precioTexto.replace(/[^0-9.]/g, ''));
    
    // Llama a la función implementada
    agregarProducto(nombre, precio, imagen, boton); 
  });
});
